...
## 接口方法上没有小鸟图标
先查看xml文件是否有小鸟图标，如果没有查看下设置->文件类型(File types)看下xml文件里面是否配置了.xml后缀名
查看xml中的namespace是否与接口一致，如果一致， 查看是否安装了其他冲突插件如FreeMybatisPlugin,MybatisX,MybatisPlugin(mybatisLogPlugin不冲突),如果有请卸卸掉 然后使用 invalidate Cache and restart. https://images.brucege.com/invalidateCacheAndRestart.png
