...
### 💡 AI 时代，插件还有用吗？有用，而且 AI 和插件可以一起工作。

AI 写代码越来越强，但 MyBatis 有个特点：**它要连真实数据库、要保证 SQL 正确性**。这两件事，AI 和插件正好互补：

| 场景 | AI 擅长 | 插件补上 |
|------|--------|--------|
| **生成一遍 CRUD** | 快速写出骨架 | 表上右键 / Alt+Enter 即刻生成，不消耗 token |
| **改个字段重新生成** | 重新描述一遍 | 数据库加字段，插件一键更新，不覆盖自定义代码 |
